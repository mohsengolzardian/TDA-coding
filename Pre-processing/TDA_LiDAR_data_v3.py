...
SHRINK_X = 0.40           # <1.0 compresses PC1 (try 0.9, 0.8, 0.7…)
SHRINK_Y = 0.40           # <1.0 compresses PC2
...
# =========================
# 6) DISPLAY TWEAKS (anisotropic shrink + overlap handling)
# =========================
# Shrink around the centroid so points look *closer* (visual only)
Z_mean = Z.mean(axis=0)
Z_disp = np.column_stack([
    Z_mean[0] + SHRINK_X * (Z[:, 0] - Z_mean[0]),
    Z_mean[1] + SHRINK_Y * (Z[:, 1] - Z_mean[1]),
])
...
# (Optional) set explicit limits instead:
