...
class tda:
    def __init__(self, homo_dim=1, fts='all') -> None:
        ...
        self.persistence = VietorisRipsPersistence(
            ...
            n_jobs=-1,
            max_edge_length=1e9  # edge length
        )
        self.fts = fts
        #  feature extractors initializing
...
# Main Code: # 1- Get the cloud from LAS file 2- extract TDA features (using random sampling)
...
my_tda = tda(homo_dim=1, fts='all')  # this function extract the features
...
# flip columns to start from zero to one
...
# saving the normalized median features for each m value to a CSV format
...
for i in range(normalized_features.shape[1]):
    # If the feature_labels list is shorter than the number of features, fallback to a generic label
...
plt.ylabel("Normalized median feature value")
plt.title("Normalized Median Consensus Features vs. Sample Size")
...
# Compute the persistence diagram on the full point cloud before saving
