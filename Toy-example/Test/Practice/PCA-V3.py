...
# Assign colors based on PC3 values
cavity_colors = plt.cm.coolwarm(normalized_pc3[pc_values[:, 2] < (mean_pc3 - 1.5 * std_pc3)])
hump_colors = plt.cm.coolwarm(normalized_pc3[pc_values[:, 2] > (mean_pc3 + 1.5 * std_pc3)])
...
# 3D Plot in Matplotlib with Color Mapping
...
ax.set_title("3D Plot with Color Mapping")
ax.set_xlabel("X")
ax.set_ylabel("Y")
ax.set_zlabel("Z")
ax.legend()
...
all_colors = np.vstack((cavity_colors[:, :3], hump_colors[:, :3]))  # Remove alpha channel
